...
class SupervisedEmploeePage {
    ...
    async EditSupervisedEmployee(firstName, lastName, telefonNumber, emailAddress) {
        await this.sharedUtils.OpenPageMancal("", true);
        await this.businessDetails.click();
        await this.page.locator('//button[text()="הצהרות"]').click();
    ...
    async AddSupervisedEmployeeCharTest(firstName = "חיים", lastName = "כהן", idNumber = "123456782", telefonNumber = "0533212321", emailAddress = null) {
        if (!emailAddress) emailAddress = this.env.email;
