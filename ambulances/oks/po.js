module.exports = {
    ...
    utils: {
        // לחיצה על מקש אנטר (ENTER) באמצעות שליחת קוד Unicode למקלדת הווירטואלית
        pressENTER: () => {
            web.pause(po.oneSecondTimeout)
            web.sendKeys('\uE007')
        },

        // לחיצה על מקש END כדי להגיע לסוף השורה או הטקסט בשדה נוכחי
        pressEND: () => {
            web.pause(po.oneSecondTimeout)
            web.sendKeys('\uE010')
        },

        // לחיצה על מקש TAB למעבר לשדה הבא (מפעיל טריגרים של יציאה משדה/Blur)
        pressTAB: () => {
            web.pause(po.oneSecondTimeout)
            web.sendKeys('\uE004')
        },

        // לחיצה על מקש BACKSPACE למחיקת התו הקודם
        pressBACKSPACE: () => {
            web.pause(po.oneSecondTimeout)
            web.sendKeys('\uE003')
        },

        // לחיצה על מקש F12 (בדרך כלל משמש לפתיחת כלי מפתחים)
        pressF12: () => {
            web.pause(po.oneSecondTimeout)
            web.sendKeys('\uE035')
        },

        // לחיצה על חץ למטה (למשל לצורך בחירה וניווט מתוך רשימה נפתחת)
        pressARROW_DOWN: () => {
            web.pause(po.oneSecondTimeout)
            web.sendKeys('\uE015')
        },

        // לחיצה על מקש DELETE למחיקת התו הבא (זה שאחרי הסמן)
        pressDELETE: () => {
            web.pause(po.oneSecondTimeout)
            web.sendKeys('\uE017')
        },
        // לחיצה על מקש HOME כדי להגיע לתחילת השורה בשדה טקסט
        pressHOME: () => {
            web.pause(po.oneSecondTimeout)
            web.sendKeys('\uE011')
        },

        // לחיצה על מקש ESCAPE לסגירת חלונות קופצים או רשימות נפתחות (Dropdowns)
        pressESC: () => {
            web.pause(po.oneSecondTimeout)
            web.sendKeys('\uE00C')
        },
    ...
    // אתחול איסוף הלוגים: עוטפת את פונקציית הלוג המקורית ושומרת את כל ההודעות שמודפסות למשתנה פנימי,
    // כך שניתן יהיה לשמור אותן לקובץ בסיום הריצה.
    ...
    // שמירת כל הלוגים שנאספו במהלך הריצה לתוך קובץ טקסט חדש בתיקייה המקומית.
    // מייצרת שם קובץ דינמי עם חותמת זמן ומחזירה את פונקציית הלוג המקורית לפעולה רגילה.
    SaveLogsToFile: function () {
        ...
        try {
            ...
            // כתיבת הנתונים שנאספו לתוך הקובץ החדש
    ...
    // קריאת קובץ נתונים המופרד בפסיקים, קידום הערכים המספריים במיקומים 0 ו-2 ב-1 (אינקרמנט),
    // דריסת הקובץ עם הנתונים המעודכנים, והחזרת השורות המעודכנות להמשך עבודה.
    ...
    // קוראת קובץ נתונים (characters.txt) המכיל מגוון רחב של תווים מיוחדים לצורך בדיקות Negative/Positive Testing.
    CharactersFile: () => {
        var fs = require('fs')
    ...
    // עוברת על מערך של שדות חובה, מבצעת ניסיון שמירה/מעבר עמוד כשהם ריקים ומוודאת שעל כל שדה מופיעה כוכבית אדומה והודעת שגיאה.
    TestRequiredFieldsBySpec: (requiredFields, pageName = "טופס") => {
        ...
        web.pause(2000)
        // ניסיון להתקדם הלאה במסך כדי לאלץ את המערכת להקפיץ שגיאות ולידציה לשדות חסרים
    ...
    Hover: () => {
        ...
        while (_attempts < 3) {
            try {
                ...
                // הזרקת סקריפט ברמת הדפדפן שגולל את האלמנט למרכז המסך ואז מפעיל עליו אירועי עכבר באופן מלאכותי
                web.execute(
                    "var el = arguments[0];" +
    ...
    // EzerOpen - פונקציית עזר לקריאת רשימת עסקים (שמות או קודים) מתוך הקובץ RP.txt.
    // שימושית כדי לשלוף באופן דינמי עסק לבדיקה במקום להשתמש בערכים הרדקודד.
    EzerOpen: () => {
    ...
    // OpenPageMancal - פתיחת עמוד 'מנכ"ל' של עסק מסוים מתוך רשימת העסקים במערכת.
    // בונה את ה-XPath לפי שם העסק הנדרש ולוחצת עליו. אם לא מועבר עסק, הפונקציה תמשוך אחד מקובץ.
    OpenPageMancal: (b = "") => {
        let bussines = ""
        let code = ""
        if (b == "") {
        ...
        web.pause(500)
        // מנגנון חלופי למקרה שהעסק שחיפשנו לא נמצא - נשלוף עסק אחר מקובץ וננסה שוב
    ...
    // OpenDetails - ניווט לעמוד פרטי עסק דרך תפריט 'מנכ"ל' ולחיצה על כפתור אקשן ספציפי (Locator המועבר אליה).
    // מפשטת את תהליך הניווט העמוק אל תוך פרטי הלקוח לבדיקות מתקדמות.
    OpenDetails: (locator) => {
        try {
            po.OpenPageMancal()
